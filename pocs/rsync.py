        ...
        if os.path.exists(target):
            ...
            if os.path.isfile(sink):
                if os.path.isfile(target):
                    ...
                    # file-???
                    logError(
                        "Target %s is neither a file nor folder (skip update)" % sink)
...
def shouldUpdate(cookie, sink, target):
    ...
    try:
        ...
        logError(
            "Fail to retrieve information about target %s (skip update)" % target)
        return 0
        ...
        try:
            try:
                if win32file:
                    ...
                    win32file.SetFileAttributesW(target, filemode & ~win32file.FILE_ATTRIBUTE_READONLY &
...
def main(args):
    ...
    opts, args = getopt.getopt(args, "qrRntuCIh", ["quiet", "recursive", "relative", "dry-run", "time", "update", "cvs-ignore", "ignore-times", "help", "delete",
                                                   "delete-excluded", "delete-from-source", "existing", "size-only", "modify-window=", "exclude=", "exclude-from=", "include=", "include-from=", "version"])
    for o, v in opts:
        if o in ["-q", "--quiet"]:
            cookie.quiet = 1
        if o in ["-r", "--recursive"]:
    ...
    for sink_root in sink_families.keys():
        ...
        files = filter(lambda x: os.path.isfile(
            os.path.join(y, x)), sink_families[sink_root])
        if files:
