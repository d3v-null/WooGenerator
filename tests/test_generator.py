...
class TestGenerator(TestCase):
    ...
    def test_populate_master_parsers(self):
        ...
        self.assertEqual(first_prod.specials, ['SP2016-08-12-ACA', 'EOFY2016-ACA'])
        ...
        if Registrar.DEBUG_MESSAGE:
            Registrar.register_message(
                SanitationUtils.coerce_bytes(spec_list.tabulate(tablefmt='simple'))
            )
        first_group = spec_list[0]
        if Registrar.DEBUG_MESSAGE:
            Registrar.register_message(
                "first group:\n%s\npformat@dict:\n%s\npformat@dir:\n%s\n" % (
                    SanitationUtils.coerce_bytes(tabulate(first_group.children, tablefmt='simple')),
