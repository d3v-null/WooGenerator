"""
CsvParseGenTree
Introduces the "Generator" structure to the heirarchical CSV parser class
CsvParseTree.

The Generator structure attaches metadata like code, name and description for
all objects.
In heirarchical structures, the metadata for an object is generated by
ananlysing its ancestors.
"""
...
class ImportGenMixin(object):
    ...
    def __str__(self):
        return "%10s <%s>" % (
            '|'.join(i for i in [self.codesum, self.namesum] if i),
            self.type_name
        )
    ...
    def join_descs(self, ancestors):
        ...
        for ancestor in reversed(ancestors_self):
            ...
            if not getattr(ancestor, 'is_item'):
                continue
            ancestor_fullname = SanitationUtils.strip_extra_whitespace(
                ancestor.fullname)
...
class CsvParseGenTree(CsvParseTree, CsvParseGenMixin):
    ...
    def __init__(self, cols, defaults, schema, **kwargs):
        ...
        assert kwargs['meta_width'] >= 2, \
            "meta_width must be greater than 2 for a GEN subclass"
