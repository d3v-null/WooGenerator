...
class ImportTreeObject(ImportObject):
    ...
    def __init__(self, *args, **kwargs):
        ...
        call_bases(
            ImportTreeObject.__bases__, '__init__', self, *args, **kwargs
        )
...
class CsvParseTree(CsvParseBase, CsvParseTreeMixin):
        ...
        if sanitized_row:
            for i in range(self.meta_width):
                ...
                try:
                    ...
                    meta[i] = ""
                    warn = UserWarning("could not get meta[{}] | {}".format(
                        i, exc
                    ))
    ...
    def get_new_obj_container(self, all_data, **kwargs):
        ...
        depth = kwargs['depth']
        assert depth is not None, \
            "depth should be available to CsvParseTree.get_new_obj_container"
    ...
    def get_kwargs(self, all_data, **kwargs):
        ...
        assert issubclass(kwargs['container'], ImportTreeObject), \
    ...
    def refresh_stack(self, rowcount, row, this_depth):
        ...
        assert len(self.stack) == this_depth, \
            "stack should have been properly refreshed"
    ...
    def process_object(self, object_data):
        ...
        # TODO: when create new special category that doesn't already exist in
        # parser, this breaks.
        assert isinstance(self.stack, ImportStack)
        oldstack = self.stack[:]
        assert isinstance(oldstack, ImportStack), \
            'stack should be ImportStack not %s' % oldstack.__class__.__name__
        self.refresh_stack_from_object(object_data)
        assert isinstance(self.stack, ImportStack), \
            'stack should be ImportStack not %s' % (
                self.stack.__class__.__name__)
        assert oldstack == self.stack, \
            'self.stack (%s) is inconsistent with %s' % (
    ...
    def verify_item(self, item_data):
        ...
        assert item_id == lookup_id, \
            "item has deviated from its place in items"
